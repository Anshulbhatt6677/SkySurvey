...
class GCPTableWidget(QFrame):
    ...
    def _init_ui(self):
        ...
        # Header & Export Bar
        top_row = QHBoxLayout()
        header = QLabel("7. Saved Points & 8. Export")
        ...
        self.table.setHorizontalHeaderLabels([
            "Point", "Latitude", "Longitude", "Height (m)", "Ant.Ht (m)",
    ...
    def refresh_table(self):
        ...
        for row_idx, p in enumerate(points):
            ...
            self.table.setItem(row_idx, 5, item_h)

            # Item 4: Antenna height removed to reach the ground mark
            item_ant = QTableWidgetItem(f"{p.antenna_height:.3f}")
            if p.antenna_height == 0.0:
                item_ant.setForeground(QColor("#F59E0B"))
                item_ant.setToolTip("No antenna height set — height is at the antenna, not the mark")
            self.table.setItem(row_idx, 4, item_ant)

            # Item 5: V.Acc
            item_v = QTableWidgetItem(f"{p.v_accuracy:.3f}")
            self.table.setItem(row_idx, 6, item_v)
            ...
            if p.quality_override:
                item_fix.setText(f"⚠ {p.fix_status}")
                item_fix.setToolTip("Saved below RTK Fixed — not survey grade")
            self.table.setItem(row_idx, 7, item_fix)

            # Item 7: Satellites
            item_sat = QTableWidgetItem(str(p.satellites))
            self.table.setItem(row_idx, 8, item_sat)

            # Item 8: Time
            item_time = QTableWidgetItem(p.timestamp.strftime("%H:%M:%S"))
            self.table.setItem(row_idx, 9, item_time)
            ...
            self.table.setCellWidget(row_idx, 10, btn_del)
